CREATE PROCEDURE [dbo].[jag_Rapport_Contrat_NombreProducteur]
...
SET NOCOUNT ON
	...
	-----------------------------------------------------------------------------------------------
	-- Ajouter GESTION DE VOLUME SEULEMENT SI PAS BESOIN DE LA PROVENANCE DU BOIS (MunicipaliteID non disponible)
	-----------------------------------------------------------------------------------------------
	if ( 
		@AgenceID_Debut is null and
		@AgenceID_Fin is null and
		@MrcID_Debut is null and
		@MrcID_Fin is null and
		@MunicipaliteID_Debut is null and
		@MunicipaliteID_Fin is null and
		@Secteur_Debut is null and
		@Secteur_Fin is null			
	   )
	Begin
		...
		GestionVolume_Detail 
			...
			Where
			((@UsineDebut is null) or (@UsineDebut <= GestionVolume.UsineID)) AND
			((@UsineFin is null) or (GestionVolume.UsineID <= @UsineFin)) AND    
			((@AnneeDebut is null) or (@AnneeDebut <= GestionVolume.Annee)) AND
			((@AnneeFin is null) or (GestionVolume.Annee <= @AnneeFin)) AND
			((@PeriodeDebut is null) or (@PeriodeDebut <= GestionVolume.Periode)) AND
			((@PeriodeFin is null) or (GestionVolume.Periode <= @PeriodeFin))    		
