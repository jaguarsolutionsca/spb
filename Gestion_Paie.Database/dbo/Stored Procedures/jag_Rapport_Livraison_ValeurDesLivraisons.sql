...
CREATE PROCEDURE [dbo].[jag_Rapport_Livraison_ValeurDesLivraisons]
...
SET NOCOUNT ON
	...
	Order by L.LivraisionID


	--------------------------------------------------------------
    -- Tableau Resume #2 by usine-Agence-mrc-mun
	--------------------------------------------------------------
	Select 
		max (F.Nom) Nom,
		...
		max(F.Rue) Rue,
		max(F.Ville) Ville,
		max(F.Code_postal) Code_postal,
		max(F.Telephone) Telephone,		
		L.ProducteurID,
	...
	--------------------------------------------------------------
    -- Tableau Resume #3 by Usine-Producteur
	--------------------------------------------------------------
	Select 
		max (F.Nom) Nom,
		max(F.AuSoinsDe) AuSoinsDe,
	...
	--------------------------------------------------------------
    -- Tableau Resume #4 by Producteur
	--------------------------------------------------------------
	Select 
		max (F.Nom) Nom,
		max(F.AuSoinsDe) AuSoinsDe,
		...
		max(F.Code_postal) Code_postal,
		max(F.Telephone) Telephone,		
		round(dbo.jag_GetSuperficieBoisee(L.ProducteurID, null),2) SuperficieBoisee,
		L.ProducteurID,
		sum(L.VolumeNet) VolumeNet,	
		sum(L.M3APP) M3APP,
		sum(L.M3SOL) M3SOL,
