CREATE PROCEDURE [dbo].[jag_Rapport_Statistique_AgenceMrcMunicipaliteSecteur]
@AgenceID_Debut VARCHAR (4)=null, @AgenceID_Fin VARCHAR (4)=null, @MrcID_Debut VARCHAR (6)=null, @MrcID_Fin VARCHAR (6)=null, @MunicipaliteID_Debut VARCHAR (6)=null, @MunicipaliteID_Fin VARCHAR (6)=null, @Secteur_Debut VARCHAR (2)=null, @Secteur_Fin VARCHAR (2)=null, @Annee INT=null, @MoisDebut INT=null, @MoisFin INT=null
AS
SET NOCOUNT ON
...
insert into @Result (_UsineID,_AgenceID,_MrcID, _MunicipaliteID, Secteur, _UtilisationID, _RepartitionID, _EssenceID, _UniteID, Total)
select UsineID, AgenceID, MrcID, MunicipaliteID, Secteur, UtilisationID, RepartitionID, _EssenceID, _UniteID, sum(Total)
from @Mois
group by  UsineID, AgenceID, MrcID, MunicipaliteID, Secteur, UtilisationID, RepartitionID, _EssenceID, _UniteID
...
max(case when Mu.Description is not null then Mu.Description else ' ' end) as [Municipalite],
Secteur,
...
FROM @Result
	...
	left outer join Municipalite Mu on Mu.ID = _MunicipaliteID
...
((@MunicipaliteID_Debut is null) or (@MunicipaliteID_Debut <= _MunicipaliteID)) AND
((@MunicipaliteID_Fin is null) or (_MunicipaliteID <= @MunicipaliteID_Fin)) AND
((@Secteur_Debut is null) or (@Secteur_Debut <= Secteur)) AND
((@Secteur_Fin is null) or (Secteur <= @Secteur_Fin)) 
GROUP BY _AgenceID, _MRCID, _MunicipaliteID,  Secteur, _UtilisationID, _RepartitionID
ORDER BY [Agence],[MRC], [Municipalite], Secteur, [Utilisation], [Repartition]
...
FROM @Result
	...
	left outer join MRC M on M.ID = _MrcID
	left outer join Municipalite Mu on Mu.ID = _MunicipaliteID
	left outer join Usine U on U.ID = _UsineID
	left outer join UsineUtilisation Ut on Ut.ID = _UtilisationID
	left outer join EssenceRepartition ER on ER.ID = _RepartitionID
where
((@AgenceID_Debut is null) or (@AgenceID_Debut <= _AgenceID)) AND
...
((@Secteur_Debut is null) or (@Secteur_Debut <= Secteur)) AND
((@Secteur_Fin is null) or (Secteur <= @Secteur_Fin)) 
GROUP BY _AgenceID, _MRCID, _MunicipaliteID,  Secteur, _UsineID, _UtilisationID, _RepartitionID
ORDER BY [Agence],[MRC], [Municipalite], Secteur, [Usine], [Utilisation], [Repartition]
...
((@Secteur_Debut is null) or (@Secteur_Debut <= Secteur)) AND
((@Secteur_Fin is null) or (Secteur <= @Secteur_Fin)) 
GROUP BY _AgenceID, _UtilisationID, _RepartitionID
ORDER BY [Agence], [Utilisation], [Repartition]
