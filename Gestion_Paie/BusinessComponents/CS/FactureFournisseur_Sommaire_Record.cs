...
namespace Gestion_Paie.BusinessComponents {
	...
	/// <summary>
	/// This class is an abstract class that represents the [FactureFournisseur_Sommaire] table. With this class
	/// you can load or update a record from the database. If you need to add or delete a record,
	/// you must use the <see cref="Gestion_Paie.BusinessComponents.FactureFournisseur_Sommaire_Collection"/> class to do so.
	/// </summary>
	public sealed class FactureFournisseur_Sommaire_Record : IBusinessComponentRecord {
		...
		/// <summary>
		/// Initializes a new instance of the FactureFournisseur_Sommaire_Record class. Use this contructor to add
		/// a new record. Then call the Add method of the Gestion_Paie.BusinessComponents.FactureFournisseur_Sommaire_Collection class to actually
		/// add the record in the database.
		/// </summary>
		public FactureFournisseur_Sommaire_Record() {
		...
		public FactureFournisseur_Sommaire_Record(string connectionString, System.Data.SqlTypes.SqlInt32 col_FactureID, System.Data.SqlTypes.SqlInt32 col_Ligne) {
			...
			if (olymarsDebugCheck == null || (string)olymarsDebugCheck == "True") {
				...
				if (DebugConnectionString != String.Empty) {
					...
					throw new System.InvalidOperationException(System.String.Format("OLYMARS: This code is not in sync anymore with [{0}]. It was generated when [{0}] version was: {2}. Current [{0}] version is: {1}", "FactureFournisseur_Sommaire", CurrentRevision, OriginalRevision));
					}
				}
			}
#endif
		...
		public bool Refresh() {
			...
			Params.spS_FactureFournisseur_Sommaire Param = new Params.spS_FactureFournisseur_Sommaire(true);
			...
			SPs.spS_FactureFournisseur_Sommaire Sp = new SPs.spS_FactureFournisseur_Sommaire();
			if (Sp.Execute(ref Param, out sqlDataReader)) {

				if (sqlDataReader.Read()) {

					if (!sqlDataReader.IsDBNull(SPs.spS_FactureFournisseur_Sommaire.Resultset1.Fields.Column_Compte.ColumnIndex)) {

						this.col_Compte = sqlDataReader.GetSqlInt32(SPs.spS_FactureFournisseur_Sommaire.Resultset1.Fields.Column_Compte.ColumnIndex);
					}
					if (!sqlDataReader.IsDBNull(SPs.spS_FactureFournisseur_Sommaire.Resultset1.Fields.Column_Montant.ColumnIndex)) {

						this.col_Montant = sqlDataReader.GetSqlDouble(SPs.spS_FactureFournisseur_Sommaire.Resultset1.Fields.Column_Montant.ColumnIndex);
					}
					if (!sqlDataReader.IsDBNull(SPs.spS_FactureFournisseur_Sommaire.Resultset1.Fields.Column_Description.ColumnIndex)) {

						this.col_Description = sqlDataReader.GetSqlString(SPs.spS_FactureFournisseur_Sommaire.Resultset1.Fields.Column_Description.ColumnIndex);
					}
					if (!sqlDataReader.IsDBNull(SPs.spS_FactureFournisseur_Sommaire.Resultset1.Fields.Column_isTaxe.ColumnIndex)) {

						this.col_isTaxe = sqlDataReader.GetSqlBoolean(SPs.spS_FactureFournisseur_Sommaire.Resultset1.Fields.Column_isTaxe.ColumnIndex);
					}
				...
				throw new Gestion_Paie.DataClasses.CustomException(Param, "Gestion_Paie.BusinessComponents.FactureFournisseur_Sommaire_Record", "Refresh");
			}
		}
		...
		public void Update() {
			...
			Params.spU_FactureFournisseur_Sommaire Param = new Params.spU_FactureFournisseur_Sommaire(false);
			Param.SetUpConnection(this.connectionString);
			Param.Param_FactureID = this.col_FactureID;
			Param.Param_Ligne = this.col_Ligne;
			...
			SPs.spU_FactureFournisseur_Sommaire Sp = new SPs.spU_FactureFournisseur_Sommaire();
			if (Sp.Execute(ref Param)) {
				...
				throw new Gestion_Paie.DataClasses.CustomException(Param, "Gestion_Paie.BusinessComponents.FactureFournisseur_Sommaire_Record", "Update");
			}		
		}
		...
		public override string ToString() {
			...
			if (this.displayName == null) {
				...
				Params.spS_FactureFournisseur_Sommaire_Display Param = new Params.spS_FactureFournisseur_Sommaire_Display();
				Param.SetUpConnection(this.connectionString);
				...
				System.Data.SqlClient.SqlDataReader sqlDataReader = null;
				SPs.spS_FactureFournisseur_Sommaire_Display Sp = new SPs.spS_FactureFournisseur_Sommaire_Display();
				if (Sp.Execute(ref Param, out sqlDataReader)) {

					if (sqlDataReader.Read()) {

						if (!sqlDataReader.IsDBNull(SPs.spS_FactureFournisseur_Sommaire_Display.Resultset1.Fields.Column_Display.ColumnIndex)) {

							this.displayName = "spS_FactureFournisseur_Sommaire_Display";
						}
					}
					...
					throw new Gestion_Paie.DataClasses.CustomException(Param, "Gestion_Paie.BusinessComponents.FactureFournisseur_Sommaire_Record", "ToString");
				}				
			}
