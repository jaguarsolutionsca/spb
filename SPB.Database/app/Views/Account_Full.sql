...
SELECT
	...
	CAST(CASE WHEN acc.ResetGuid IS NOT NULL AND acc.ResetExpiry IS NOT NULL AND acc.ResetExpiry < GETDATE() AND acc.LastActivity IS NULL AND acc.Archive = 0 THEN 1 ELSE 0 END AS bit) [canExtendInvitation],
	CAST(CASE WHEN acc.ResetGuid IS NOT NULL AND acc.ResetExpiry IS NOT NULL AND acc.LastActivity IS NOT NULL AND acc.Archive = 0 THEN 1 ELSE 0 END AS bit) [canResetPassword],
	CAST(CASE WHEN acc.ResetGuid IS NULL AND acc.ResetExpiry IS NULL AND acc.LastActivity IS NULL AND acc.Archive = 1 THEN 1 ELSE 0 END AS bit) [canCreateInvitation],
	acc.CurrentYear,
	acc.Comment, 
	acc.Archive, 
	acc.Created, 
