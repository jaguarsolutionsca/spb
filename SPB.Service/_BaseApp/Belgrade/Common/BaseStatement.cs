...
namespace Belgrade.SqlClient.Common
{
    public abstract class BaseStatement
    {
        ...
        public virtual BaseStatement AddLogger(ILogger logger)
        {
            this._logger = logger;
            return this;
        }
        ...
        protected async Task ExecuteWithRetry(DbCommand command, object callback)
        {
            ...
            do
            {
                ...
                catch (Exception ex)
                {
                    ...
                    if (SqlDb.RetryErrorHandler.ShouldRetry(ex) && !isResultSentToCallback)
                    {
                        ...
                        retryIteration++;
                        if (_logger != null)
                            _logger.LogInformation("Database call returned the transient error {message}. Retrying..", ex.Message);
                    }
                    else
                    {
                        if (_logger != null)
                            _logger.LogWarning("Database call returned the error {message}. Trying to handle exception...", ex.Message);
                ...
                finally
                {
                    ...
                    if (shouldRetry && !isResultSentToCallback)
                    {
                        ...
                        {
                            if (_logger != null)
                                _logger.LogInformation("Delayed retry {iteration} due to the transient error.", retryIteration);
                            await Task.Delay(5000 + 10000 * (retryIteration - 1)).ConfigureAwait(false); // wait 5, 15, and 25 sec
                        }
                        else
                        {
                            if (_logger != null)
                                _logger.LogInformation("Retrying immediatelly {iteration} attempt", retryIteration);
                        }
                    }
                }
            ...
            if (shouldRetry && retryIteration == SqlDb.RetryErrorHandler.RETRY_COUNT)
            {
                if (this._logger != null)
                    this._logger.LogError("Query failed after {iteration} retries.\n{excetpion}", SqlDb.RetryErrorHandler.RETRY_COUNT, rootException);

                await ExecuteCallbackWithException(callback, rootException);
            }
        }

        protected virtual async Task ExecuteCallbackWithException(object callback, Exception ex) { await Task.CompletedTask; }
        protected virtual async Task<bool> ExecuteCommand(DbCommand command, object callback) { return await Task.FromResult(false); }
    }
}
