...
namespace Belgrade.SqlClient.Common
{
        ...
        where T : DbCommand, new()
    {
        ...
        public override BaseStatement AddLogger(ILogger logger)
        {
        ...
        protected override Task ExecuteCallbackWithException(object callback, Exception ex)
        {
            if (_logger != null)
                _logger.LogError("Error occurred while trying to provide query results to mapper function.", ex);
            try
            {
            ...
            {
                if (_logger != null)
                    _logger.LogError("Callback provided to Map() thrown the error while trying to handle exception.", ex2);
                throw;
            }
        }
