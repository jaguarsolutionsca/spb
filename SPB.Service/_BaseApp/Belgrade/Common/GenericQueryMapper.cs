...
namespace Belgrade.SqlClient.Common
{
    ...
    /// </summary>
    public class GenericQueryMapper<T> : BaseStatement, IQueryMapper where T : DbCommand, new()
    {
        /// <summary>
        ...
        {
            if (_logger != null)
                _logger.LogError("Error {message} occurred while trying to provide query results to mapper function. \n{exception}", ex.Message, ex);
            try
            {
            ...
            {
                if (_logger != null)
                    _logger.LogError("Callback provided to Map() thrown the error {error} while trying to handle exception.\n{exception}", ex2.Message, ex2);
                throw;
            }
        }
