...
namespace Belgrade.SqlClient.Common
{
    ...
    /// </summary>
    public class GenericQueryPipe<T> : GenericQueryMapper<T>, IQueryPipe where T : DbCommand, new()
    {
        /// <summary>
        ...
        private async Task FlushSqlResultsToStream<TOutput>(DbCommand command, TOutput stream, Options options /*byte[] defaultOutput*/)
        {
                    ...
                    async reader =>
                    {
                        try
                        {
                            ...
                            if (reader.HasRows)
                            {
                                ...
                                {
                                    if (_logger != null)
                                        _logger.LogCritical("Column type " + reader[0].GetType().Name + " cannot be sent to the streamed.");
                        ...
                        {
                            if (_logger != null)
                                _logger.LogError("Error {error} occured while trying to flush results of SQL query to the stream.\n{exception}", ex.Message, ex);
                            ...
                            {
                                if (_logger != null)
                                    _logger.LogError("Error {error} occured while trying to handle error in query pipe error handler on {source}.", ex2.Message, ex2.Source);
                                throw;
                            }
                        }
                    });
            ...
            {
                if (_logger != null)
                    _logger.LogError("Error occured while trying to map results to output.", ex);
                ...
                {
                    if (_logger != null)
                        _logger.LogWarning("Error occured while trying to handle error in query pipe error handler(2).", ex2);
                    throw;
                }
            }
            finally
        ...
        /// <returns>Task</returns>
        private static async Task FlushContent<TOutput>(TOutput output, object content, ILogger _logger, Encoding outputEncoding, int amount = -1)
        {
            try
            {
            ...
            {
                if (_logger != null)
                    _logger.LogWarning("Error occured while trying to flush content " + content + " to output " + output.GetType().Name, ex);
                throw;
            }
        }
